...
function principalPath(route) { return route === '/' ? path.join(root,'index.html') : path.join(root,route.slice(1),'index.html'); }
...
for (const route of principalRoutes) {
  ...
  if (!fs.existsSync(file)) { failures.push(`Missing principal route: ${route}`); continue; }
  ...
  if (!header.includes(foundationMarker) || !footer.includes(foundationMarker)) failures.push(`${route}: foundation v13.1 global marker missing`);
  if (!html.includes('/foundation-mobile-hotfix.css?v=13.1')) failures.push(`${route}: mobile foundation hotfix missing`);
  ...
  if (/axante-logo\.svg|www\.axante\.it\/wp-content\/uploads\/2021\/08\/axante-logo\.png/i.test(header + footer)) failures.push(`${route}: global components still reference a legacy logo`);
}

for (const file of htmlFiles) {
  const relative = path.relative(root,file);
  const html = fs.readFileSync(file,'utf8');
  for (const token of ['<meta name="viewport"','<meta name="description"','<title>','<h1']) if (!html.includes(token)) failures.push(`${relative}: missing ${token}`);
  ...
  if (/src="\/assets\/axante-logo\.svg"|src="https:\/\/www\.axante\.it\/wp-content\/uploads\/2021\/08\/axante-logo\.png"/i.test(html)) failures.push(`${relative}: legacy Axante logo reference survived foundation pass`);
...
for (const [primary,detail] of [['casarossa.jpg','casarossa-detail.jpg'],['unicart.jpg','unicart-detail.jpg'],['carabetta.jpg','carabetta-detail.jpg']]) {
...
if (fs.existsSync(manifestPath)) {
  try {
    ...
    if (manifest.version !== '13.1') failures.push(`asset-manifest.json: expected version 13.1, found ${manifest.version}`);
...
if (fs.existsSync(socialPath)) {
  try {
    ...
    if (metadata.width!==1200 || metadata.height!==630 || metadata.format!=='png') failures.push(`Social image has invalid metadata: ${metadata.width}x${metadata.height} ${metadata.format}`);
  } catch(error) { failures.push(`Unable to inspect social image: ${error.message}`); }
}
